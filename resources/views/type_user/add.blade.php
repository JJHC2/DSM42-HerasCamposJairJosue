...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Tipo Usuario Altas</h1>
</div>
...
<div class="row">
    ...
    <div class="col-lg-12 mb-4">
        ...
        <div class="card shadow mb-4">
            ...
            <form action="{{ url('type_user') }}" id="agregar" method="post">
            {!! csrf_field() !!}
                <label >Tipo:</label>
                <select class="form-control form-select" aria-label="Default select example" name="type" required>
                  <option>Administrador</option>
                  <option>Usuario</option>
                </select>
                <div class="form-group">
                <label for="">Usuario:</label>
                    <select class="form-control form-select" aria-label="Default select example" name="users_id" required>
                    <option selected>Elige el Usuario</option>
                        @foreach($usuarios as $usua)   
                    <option value={{$usua->id}}>{{$usua->name}}</option>
                       @endforeach
                    </select>
                </div>
                <div class="row">
